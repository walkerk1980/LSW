...
void GuestTelemetryLogger::Start(const wil::unique_event& ExitEvent)
{
    ...
    m_pipeName = lsw::windows::common::helpers::GetUniquePipeName();
    ...
    wil::unique_handle exitEvent(lsw::windows::common::helpers::DuplicateHandle(ExitEvent.get()));
    m_thread = std::thread([Self = shared_from_this(), Pipe = std::move(pipe), ExitEvent = std::move(exitEvent)]() {
        try
        {
            lsw::windows::common::lswutil::SetThreadDescription(L"GuestTelemetryLogger");

            // When the pipe connects, start reading data.
            const std::vector<HANDLE> exitEvents = {Self->m_threadExit.get(), ExitEvent.get()};
            lsw::windows::common::helpers::ConnectPipe(Pipe.get(), INFINITE, exitEvents);
            ...
            for (;;)
            {
                ...
                const auto bytesRead =
                    lsw::windows::common::relay::InterruptableRead(Pipe.get(), gsl::make_span(buffer), exitEvents, &overlapped);
...
void GuestTelemetryLogger::ProcessInput(const std::string_view Input)
{
    ...
    {
        LSW_LOG("GuestTelemetry", TraceLoggingValue(logLine.c_str(), "text"), TraceLoggingValue(m_runtimeId, "vmId"));
    }
}